...
#if ((defined(TSLOG) || defined(TSLOG2) || defined(TSLOG_COMPLEX)) && (defined(_WIN64) || defined(WIN64)))
#ifndef TSLOG_SIMPLE
#define TSLOG_SIMPLE
#endif 
#pragma message("warning: define macro 'TSLOG', 'TSLOG2'/'TSLOG_COMPLEX', and define macro '_WIN64' or define macro 'WIN64', default : [#define TSLOG_SIMPLE]")
#endif
...
#define TSLOG_CONFIG_DIR "%temp%\\GreenShield\\GSCFG\\"
...
//no config, default
#define DEFAULT_LOGFILE_PATH				"%temp%\\GSLOG\\"
...
private:
	...
	{		
		...
		if (QueryPerformanceFrequency(&nFreq))
        {
			...
			return (ULONGLONG)nLastTime1.QuadPart;
		}
		else
		{
...
__declspec(selectany) volatile LONG CTSLog::s_lOperatorStarted = 0;
...
{	
	...
	if(0 == _tcsicmp(szReturnedString, _T("{C93F597A-E01B-4c62-91A8-FEA33D1C53C9}")) || _tcsicmp(szReturnedString, _T("{C93F597A-E01B-4c62-91A8-FEA33D1C53C9}")) == 0)
		m_bDebugViewLog = TRUE;
	...
	if(0  == _tcsicmp(szReturnedString, _T("{C93F597A-E01B-4c62-91A8-FEA33D1C53C9}")) || 0 == _tcsicmp(szReturnedString, _T("{C93F597A-E01B-4c62-91A8-FEA33D1C53C9}")))
		m_bFileLog = TRUE;
	else if(0 == _tcsicmp(szReturnedString, _T("OFF")) || 0 == _tcsicmp(szReturnedString, _T("0")))
		m_bFileLog = FALSE;
	if(bWriteConfig)
...
void CTSLog::OutputLog(TLevel level, LPCTSTR pszFormat, va_list ap, LPCTSTR pszLogFilePath, BOOL bCheckFileChanged)
{
	...
	if(bCheckFileChanged)
	{
		...
		_pdwThis = (LPDWORD) TlsGetValue(s_dwTlsIndex_this);		
		CheckConfigFileChanged();
		TlsSetValue(s_dwTlsIndex_this, _pdwThis);
...
void CTSLog::TSLog(LPCTSTR pszLogFileName,LPCTSTR pszFormat, ...)
{
	SectionProtect sp(& _cs); 
	LPCTSTR pszLogFilePath = NULL;
	if(pszLogFileName)
	{		
		TCHAR szLogFilePath[_MAX_PATH] = {0};		
#if (defined(UNICODE) || defined(_UNICODE))		
		std::basic_string<WCHAR>strLogFilePathW;
		MultiByteToUnicode(m_szLogFileDirA,strLogFilePathW );		
		_sntprintf(szLogFilePath, _MAX_PATH, _T("%s%s.%s.log"), strLogFilePathW.c_str(), __TTSLOG_GROUP, pszLogFileName);	
#else
		_sntprintf(szLogFilePath, _MAX_PATH, _T("%s%s.%s.log"), m_szLogFileDirA, __TTSLOG_GROUP, pszLogFileName);		
#endif	
...
#define MAX_ALL_HEX_DUMP_SIZE MAX_HEX_DUMP_COUNT * MAX_HEX_DUMP_SIZE
	...
	if(NULL == pNext)
	{
		...
		_sntprintf(szDumpHead, MAX_HEX_DUMP_SIZE, _T("TSLOG MEMORY HEXDUMP Begin [0x%p], Total Lenght : [%lu] bytes, Prompt : %s"), p, uSize, pszPrompt);
...
#define TOPRINTCHAR(X)  isprint((int)X) && X != '%' ? X : '.'
	for (i = 0; i < cbWLineCount; i++, pbyData += 16,pszData += 16)
	{
		__try
		{			
			TCHAR* pszTransCharSet = NULL;
			TCHAR szTransCharset [20] = {0};
			pszTransCharSet = szTransCharset;			
			memcpy(szTransCharset, &pszData[0], 16);
			for (int i = 0; i <16 ; i++)
			{
				if ('\0' == pszTransCharSet[i] || '\r' == pszTransCharSet[i] || '\n' == pszTransCharSet[i])
					pszTransCharSet[i] = _T('.');
			}
			cbDataSize += _sntprintf(&szHex[cbDataSize], MAX_PRIVATEDATA_SIZE + MAX_USERDATA_SIZE - cbDataSize, 
				_T("00000000\t00:00:00.000\t[0000] 0x%p %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %3s %s\r\n"),pbyData,
...
#define  _TSTRACEAUTO() \
	...
	__if_not_exists(this){tslog_lpctszFuncSig = CTSLog::TraceAuto(__FUNCSIG__,tslog_dwEBP,TRUE);if(!tslog_lpctszFuncSig) tslog_lpctszFuncSig = __TFUNCSIG__; CTSLog::CEnterLeaveFunCallBack callback(tslog_lpctszFuncSig,ENTER_FUNCTION_FLAG __TFUNCTION__, __TFUNCSIG__ , NULL); } \
...
#define  _TSTRACEAUTO2() \
	...
	__if_not_exists(this){ if(CTSLog::IsTRACEValid()) { tslog_lpctszFuncSig = CTSLog::TraceAuto(__FUNCSIG__,tslog_dwEBP,TRUE);if(!tslog_lpctszFuncSig) tslog_lpctszFuncSig = __TFUNCSIG__; CTSLog::CEnterLeaveFunCallBack callback(tslog_lpctszFuncSig,ENTER_FUNCTION_FLAG __TFUNCTION__, __TFUNCSIG__, NULL); } } \
	if(!tslog_lpctszFuncSig){{__asm push ebp }{__asm pop ebp  }} //no reach, but absolutely necessarily. 
...
private:
	...
	volatile LONG s_lOperatorStarted;
public:
	//V2.0 重载<<
#define OVERFLOW_CHECK(x)  \
	if((DWORD)-1 == x) \
...
#define OPERATOR_XX_AUTOGUARD() \
	...
	inline  CTSLog& 		operator << (const bool _Val)
	{
		...
		if(m_dwUsedSize < m_dwMaxLogDataSize)
		{
			int n = 0;
			if(_Val)
				n = _sntprintf(&m_pszLogDataA[m_dwUsedSize], m_dwMaxLogDataSize - m_dwUsedSize, _T("true"));
			else
				n = _sntprintf(&m_pszLogDataA[m_dwUsedSize], m_dwMaxLogDataSize - m_dwUsedSize, _T("false"));
			if(0 < n)
				m_dwUsedSize += n;
			OVERFLOW_CHECK(n);
		}		
