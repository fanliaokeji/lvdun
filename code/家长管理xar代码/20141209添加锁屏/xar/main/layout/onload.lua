...
function RegisterFunctionObject(self)
	...
	obj.HasRegsiter = HasRegsiter
	obj.CheckUserKey = CheckUserKey
	obj.SetUserKey = SetUserKey
	obj.EncryptAesBase64 = EncryptAesBase64
	obj.GetUserPhone = GetUserPhone
	obj.SetUserPhone = SetUserPhone
...
function GetKeyMaterialRegPath()
	return "HKEY_LOCAL_MACHINE\\Software\\GreenShield\\Material"
end

function GetPhoneRegPath()
	return "HKEY_LOCAL_MACHINE\\Software\\GreenShield\\EncPh"
end


function GetUserKey()
	local strKeyPath = GetKeyMaterialRegPath()
	local strEncKey = RegQueryValue(strKeyPath)
	return strEncKey
end


function SetUserKey(strEncKey)
	local strKeyPath = GetKeyMaterialRegPath()
	local bRet = RegSetValue(strKeyPath, strEncKey)
	return bRet
end

function GetUserPhone()
	local strPhonePath = GetPhoneRegPath()
	local strEncPhone = RegQueryValue(strPhonePath)
	return strEncPhone
end

function SetUserPhone(strEncPhone)
	local strPhonePath = GetPhoneRegPath()
	local bRet = RegSetValue(strPhonePath, strEncPhone)
	return bRet
end

function HasRegsiter()
	local strKey = GetUserKey()
	if IsRealString(strKey) then
		return true
	else
		return false
	end
...
function CheckUserKey(strOriKey)
	if not IsRealString(strOriKey) then
		return false
	end

	local strLocalUserKey = GetUserKey()
	if not IsRealString(strLocalUserKey) then
		return true
	end
	
	local strEncKey = tostring(EncryptAesBase64(strOriKey))
	if strEncKey == strLocalUserKey then
		return true
	else
		return false
	end	
...
function EncryptAesBase64(strOri)
	if not IsRealString(strOri) then
		return ""
	end

	local strKey = "7uxSw0inyfnTawtg"
	local strEnc = tipUtil:Encrypt_AES_BASE64(strOri,strKey)
