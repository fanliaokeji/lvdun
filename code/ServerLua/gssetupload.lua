...
function RegisterFunctionObject(self)	
	local function TipConvStatistic(tStat)
		...
		apiAsyn:AsynSendHttpStat(strUrl, function()
			gStatCount = gStatCount - 1
		...
		if gForceExit and iStatCount > 0 and gTimeoutTimerId == nil then	--开启定时退出定时器
			gTimeoutTimerId = apiAsyn:SetTimer(function(id)
				apiAsyn:KillTimer(id)
				gTimeoutTimerId = nil
				ExitTipWnd()
...
function Main()
	...
	strSavePath = apiUtil:PathCombine(strSavePath,"GSSetupAddin.dat") 
