...
function AnimChangePos(self, bLeave)
	local attr = self:GetAttribute()
	local uiBottomText = self:GetControlObject("Recomment")
	if attr.PosAnim then
		attr.PosAnim:Stop()
		attr.PosAnim = nil
	end
	local aniFactory = XLGetObject("Xunlei.UIEngine.AnimationFactory")
	local id = self:GetID()
	attr.PosAnim = aniFactory:CreateAnimation("PosChangeAnimation")
	local bolder = self:GetControlObject("Board")
	local l, t, r, b = bolder:GetObjPos()
	attr.PosAnim:BindObj(bolder)
	attr.PosAnim:SetTotalTime(200)
	local nTarget
	if bLeave then
		nTarget = 0
		--[[attr.PosAnim:AttachListener(true, function(anim, old, new)
			if new == 3 or new == 4 then
				uiBottomText:SetVisible(true)
			end
		end)]]--
		if nTarget ~= t then
			uiBottomText:SetVisible(true)
		...
		nTarget = bottom - top - 15 - attr.ImgHeight
		uiBottomText:SetVisible(false)
	end
	attr.PosAnim:SetKeyFramePos(l, t, l, nTarget)
	local owner = self:GetOwner()
	owner:AddAnimation(attr.PosAnim)
	attr.PosAnim:Resume()
end
...
function OnMouseMove(self, x, y)
	...
	if attr.Enable then
		if attr.NowState == 0 then
			self:SetState(1)
			AnimChangePos(self, true)
		elseif attr.NowState ~= 1 and attr.NowState ~= 2 then
			self:SetState(0)
			AnimChangePos(self, false)
			self:SetCaptureMouse(false)
			attr.CaptrueMouse = false
		end
	end
...
function OnInitControl(self)
    local attr = self:GetAttribute()
	if type(attr.ImgHeight) == "number" then
		local bolder = self:GetControlObject("Board")
		bolder:SetObjPos(0, "father.height-15-"..attr.ImgHeight, "father.width", "father.height-15")
	end
...
function SetTextStyle(objRootCtrl)
	...
	textObj:SetObjPos(nLeftPos, nTopPos, "father.width", "father.height")
end
