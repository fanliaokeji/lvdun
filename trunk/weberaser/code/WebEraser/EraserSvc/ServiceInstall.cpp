...
static const wchar_t* szServiceName = L"WebEraserService";
...
HRESULT CreateGreenShieldService(const wchar_t* szDllPath)
{
	...
	const wchar_t* szImagePath = L"%SystemRoot%\\System32\\svchost.exe -k WebEraserService";
	...
	SERVICE_DESCRIPTION description = { L"广告橡皮差过滤服务。" };
	ChangeServiceConfig2(schService, SERVICE_CONFIG_DESCRIPTION, &description);

	// HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\WebEraserService
	ATL::CRegKey key;
	LONG lRegResult = key.Open(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\services\\WebEraserService", KEY_READ);
	if(lRegResult != ERROR_SUCCESS) {
		TSERROR4CXX("Filed to open reg key. Key: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\services\\WebEraserService. Error: " << lRegResult);
		return HRESULT_FROM_WIN32(lRegResult);
	}

	key.Close();

	// HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\services\WebEraserService\Parameters
	lRegResult = key.Create(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\services\\WebEraserService\\Parameters");
	if(lRegResult != ERROR_SUCCESS) {
		TSERROR4CXX("Failed to create reg key. Key: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\services\\WebEraserService\\Parameters. Error: " << lRegResult);
		return HRESULT_FROM_WIN32(lRegResult);
	}
	lRegResult = key.SetStringValue(L"ServiceDll", szDllPath, REG_EXPAND_SZ);
	if(lRegResult != ERROR_SUCCESS) {
		TSERROR4CXX("Failed to set reg value. Key: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\services\\WebEraserService\\Parameters\\ServiceDll. Error: " << lRegResult);
		return HRESULT_FROM_WIN32(lRegResult);
	}
	key.Close();
	...
	lRegResult = key.SetMultiStringValue(L"WebEraserService", L"WebEraserService\0");
	if(lRegResult != ERROR_SUCCESS) {
		TSERROR4CXX("Failed to set reg value. Key: HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Svchost\\WebEraserService. Error: " << lRegResult);
		return HRESULT_FROM_WIN32(lRegResult);
	}
...
bool CopyFilesToPublicFolder()
{
	...
	// WebEraser\\addin
	const wchar_t* addinSuffix = L"WebEraser\\addin\\";
	...
	wchar_t* serviceDll = L"EraserSvc.dll";
...
HRESULT SetupInstallService()
{
	...
	// WebEraser\\addin\\EraserSvc.dll
	const wchar_t* addinSuffix = L"WebEraser\\addin\\EraserSvc.dll";
	std::size_t addinSuffixLength = std::wcslen(addinSuffix);
...
HRESULT UninstallService()
{
	...
	if(lRegResult == ERROR_SUCCESS) {
		lRegResult = key.DeleteValue(L"WebEraserService");
		if(lRegResult != ERROR_SUCCESS) {
			TSWARN4CXX("Failed to delete reg value. Key: HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Svchost\\WebEraserService. Error: " << lRegResult);
		}
		key.Close();
	}
	else {
	...
	lRegResult = key.Open(HKEY_LOCAL_MACHINE, L"SYSTEM\\CurrentControlSet\\services\\WebEraserService");
	if(lRegResult == ERROR_SUCCESS) {
		lRegResult = key.RecurseDeleteKey(L"Parameters");
		if(lRegResult != ERROR_SUCCESS) {
			TSWARN4CXX("Failed to delete reg key. Key: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\services\\WebEraserService\\Parameters. Error: " << lRegResult);
		}
		key.Close();
	}
	else {
		TSWARN4CXX("Failed to open reg key. Key: HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\services\\WebEraserService. Error: " << lRegResult);
	}
