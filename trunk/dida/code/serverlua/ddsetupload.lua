...
function ExitTipWnd()	
	Log("************ Exit ************")
	apiAsyn:SoftExit("Exit")
end

function DecreaseExecuteFlowNum(str)
	Log("[DecreaseExecuteFlowNum] " .. tostring(str))
	nCloudNum = nCloudNum -1
	if nCloudNum<= 0 then
		ExitTipWnd()
	end	
end

function IncreaseExecuteFlowNum(str)
	Log("[IncreaseExecuteFlowNum] " .. tostring(str))
	nCloudNum = nCloudNum+1
end

function NewAsynSendHttpStat(strUrl, funCallback)
	apiAsyn:AjaxSendHttpStat(strUrl, funCallback)
end

function NewAsynSendHttpStatOnlyOnce(strUrl)
	local timerID
	local bHasAlreadyCallback = false
	IncreaseExecuteFlowNum("NewAsynSendHttpStatOnlyOnce(" .. tostring(strUrl) .. ")")
	NewAsynSendHttpStat(strUrl, 
		function (ret)
			if timerID ~= nil then
				apiAsyn:KillTimer(timerID)
			end
			if not bHasAlreadyCallback then
				bHasAlreadyCallback = true
				DecreaseExecuteFlowNum("NewAsynSendHttpStatOnlyOnce_Callback(" .. tostring(strUrl) .. ")")
			end
		end)
		
	timerID = apiAsyn:SetTimer(3 * 60 * 1000,
		function (nTimerId)
			apiAsyn:KillTimer(nTimerId)
			timerID = nil
			if not bHasAlreadyCallback then
				bHasAlreadyCallback = true
				DecreaseExecuteFlowNum("NewAsynSendHttpStatOnlyOnce_Callback_Timeout(" .. tostring(strUrl) .. ")")
			end
		end)
end

function NewAsynGetHttpFile(strUrl, strSavePath, bDelete, funCallback, nTimeoutInMS)
	IncreaseExecuteFlowNum("NewAsynGetHttpFile(" .. tostring(strUrl) .. ")")
	local bHasAlreadyCallback = false
	local timerID = nil
	apiAsyn:AsynGetHttpFile(strUrl, strSavePath, bDelete, 
		function (nRet, strTargetFilePath, strHeaders)
			...
			if not bHasAlreadyCallback then
				...
				DecreaseExecuteFlowNum("NewAsynGetHttpFile(" .. tostring(strUrl) .. ")")
			end
		end)
	
	timerID = apiAsyn:SetTimer(nTimeoutInMS or 2 * 60 * 1000,
		function (nTimerId)
			apiAsyn:KillTimer(nTimerId)
			timerID = nil
			if not bHasAlreadyCallback then
...
function RegQueryValue(sPath)
	if IsRealString(sPath) then
		...
		if IsRealString(sRegRoot) and IsRealString(sRegPath) then
			return apiUtil:QueryRegValue(sRegRoot, sRegPath, sRegKey or "") or ""
		end
	end
	return ""
end
...
function GetTimeStamp()
	...
	local iTime = apiUtil:GetCurrentUTCTime()
...
function RegisterFunctionObject(self)	
	local function TipConvStatistic(tStat)
		local rdRandom = apiUtil:GetCurrentUTCTime()
		...
		NewAsynSendHttpStatOnlyOnce(strUrl)
	end
	...
	obj.IncreaseExecuteFlowNum = IncreaseExecuteFlowNum
	obj.DecreaseExecuteFlowNum = DecreaseExecuteFlowNum
	obj.GetFileSaveNameFromUrl = GetFileSaveNameFromUrl
	XLSetGlobal("Setup.FunctionHelper", obj)
end

function Main(strParam)
	IncreaseExecuteFlowNum("Enter main")
	RegisterFunctionObject()
	local FunctionObj = XLGetGlobal("Setup.FunctionHelper")
	local strStamp = GetTimeStamp()
	local strURLFix = strServerLua..strStamp
	local nTime = 60*1000
	...
	, function(bRet, strRealPath)
		...
		if 0 == bRet then
			...
			local tStatInfo = {}
			tStatInfo.strEC = "ddsettuplua"
			tStatInfo.strEA = "downaddinfail"
			tStatInfo.strEV = 1
			tStatInfo.Exit = true
			local FunctionObj = XLGetGlobal("Setup.FunctionHelper")
			FunctionObj.TipConvStatistic(tStatInfo)
		end	
		DecreaseExecuteFlowNum("Exit Main")
	end, nTime)
end