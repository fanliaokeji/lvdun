...
----------------------debug
function BeginLock()
	local bFunSucc, bHasLoad = LoadXar("lock")
	if not bFunSucc then
		return
	end	
...
function ShowLockWnd()	
	local hostwndManager = XLGetObject("Xunlei.UIEngine.HostWndManager")
	local strLockWndName = "GreenWallLockWnd.MainFrame"
	local objLockWnd = hostwndManager:GetHostWnd(strLockWndName)
	if not objLockWnd then
		return
	end
	
	objLockWnd:Show(5)
	tFunctionHelper.SetWndForeGround(objLockWnd)
end


function LoadXar(xarName)
	local bFunSucc = true
	local bHasLoad = true

	local xarManager = XLGetObject("Xunlei.UIEngine.XARManager")
	if nil == xarManager then
		return not bFunSucc
	end
	local loaded = xarManager:IsXARLoaded(xarName)
	local exist = xarManager:XARExist(xarName)
	if loaded or not exist then
		return bFunSucc, bHasLoad
	end
	xarManager:LoadXAR(xarName)
	return bFunSucc, not bHasLoad
end
