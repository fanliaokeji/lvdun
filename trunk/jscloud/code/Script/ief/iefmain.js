...
function CommonFun.RegSetValue(strRegPath, value, valType)
{
	return __TRY( 
				function()
				{
					//路径不存在，要先创建之
					return __WshShell.RegWrite(strRegPath, value, valType)
				}
			)
}
function CommonFun.RegDelValue(strRegPath)
{
	return __TRY( 
				function()
				{
					return __WshShell.RegDelete(strRegPath)
				}
			)
}

function CommonFun.IsWin7()
{
	var sysVersion =  CommonFun.RegQueryValue("HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\CurrentVersion")
	
	if (parseFloat(sysVersion) > 6.0 )
	{	
		return true
	}
	return false
...
function CommonFun.GetReportURL(tReport)
{
	...
	var strEL = tReport.EL ? tReport.EL : __ThisJSVersion
