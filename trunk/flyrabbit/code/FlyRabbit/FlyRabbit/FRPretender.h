...
{
public:
	CFRPretender() : m_bInitOK(FALSE), m_hFRKernel(NULL)
	{
		...
		m_hFRKernel = LoadLibrary(_T("FRKernel.dll"));
		if (NULL == m_hFRKernel)
		{
			return;
		}
	...
	BOOL Init(LPTSTR lpCmdLine = NULL)
	{
		...
		if (m_bInitOK)
		{
			...
			PHandleSingleton pHandleSingleton = (PHandleSingleton)GetProcAddress(m_hFRKernel,"HandleSingleton");
			...
			PInitXLUE pInitXLUE = (PInitXLUE)GetProcAddress(m_hFRKernel, "InitXLUE");
			if (pInitXLUE)
			{
