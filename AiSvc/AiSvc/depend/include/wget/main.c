  ...
  //for (t = url; *t; t++)
    {
      ...
      status = retrieve_url (url, &filename, &new_file, NULL, &dt,localFile);
      if (opt.recursive && status == RETROK && (dt & TEXTHTML))
	status = recursive_retrieve (filename, new_file ? new_file : url);
      FREE_MAYBE (new_file);
      FREE_MAYBE (filename);
    }
...
int DownLoadBuff (char* url, char *pBuff,DWORD dwSize)
{
  ...
  //for (t = url; *t; t++)
    {
      ...
      status = retrieve_url_buff (url, &filename, &new_file, NULL, &dt,pBuff,dwSize);
      if (opt.recursive && status == RETROK && (dt & TEXTHTML))
	status = recursive_retrieve (filename, new_file ? new_file : url);
      FREE_MAYBE (new_file);
      FREE_MAYBE (filename);
  ...
  for (t = url; *t; t++)
    {
      ...
      status = retrieve_url (*t, &filename, &new_file, NULL, &dt,NULL);
