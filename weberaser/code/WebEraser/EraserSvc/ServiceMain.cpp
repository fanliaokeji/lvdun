    ...
    while(1) {
		...
		if(hMutex == NULL) {
			if(::IsVistaOrLatter()) {
						...
						if (::ConvertStringSecurityDescriptorToSecurityDescriptor(_T("S:(ML;;NW;;;LW)"), SDDL_REVISION_1, &pSD, NULL)) {
							...
							if(::GetSecurityDescriptorSacl(pSD, &fSaclPresent, &pSacl, &fSaclDefaulted)) {
								if(::SetSecurityDescriptorSacl(sa.lpSecurityDescriptor, TRUE, pSacl, FALSE)) {
									hMutex = ::CreateMutex(&sa, TRUE, L"Global\\{D8BD00DC-74BF-45ad-B8CB-61CB31C2CE85}-eraseraddin");
				...
				hMutex = ::CreateMutex(NULL, TRUE, L"Global\\{D8BD00DC-74BF-45ad-B8CB-61CB31C2CE85}-eraseraddin");
				if(hMutex != NULL && ::GetLastError() == ERROR_ALREADY_EXISTS) {
					::CloseHandle(hMutex);
					hMutex = NULL;
				}
